...
#Select Player
player_analysed <- "Philippe Coutinho"

#----Create Passing Map----

#Filter to only include key passes from correct match and player
passes_map_data <- match_event_data %>%
  filter(player.name == player_analysed & (pass.shot_assist == TRUE | pass.goal_assist == TRUE)) %>%
  #create variable to determine whether pass assisted a goal
...
create_Pitch(goaltype = "box",
  ...
  #Add titles
  labs(title = player_analysed, subtitle = "Key Pass Map, Premier League, 2015/16" , colour = "Pass Outcome") +
  scale_y_reverse() +
